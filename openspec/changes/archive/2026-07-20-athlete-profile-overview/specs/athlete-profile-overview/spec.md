...
When a latest `ATHLETE_PROFILE` report is available via Bearer API, the overview SHALL show the executive summary text and SHALL show athlete score chips when score fields are present. When no report exists, the overview SHALL show an honest empty state with Open web (and Sync when generate is available) rather than placeholder AI copy.
...
#### Scenario: Reports API unavailable
- **WHEN** Bearer reports access is not available (not implemented or 401/403)
- **THEN** the overview does not claim AI text is loading forever and offers Open web to `/profile/athlete`
...
The overview SHALL offer Open web that opens the instance `/profile/athlete` (or equivalent Athlete Profile report path) in the system browser for the full AI report and web-only tools.

#### Scenario: Open web report
- **WHEN** the athlete chooses Open web from the Athlete profile overview
- **THEN** the system browser opens the instance Athlete Profile report surface
