...
> = {
  ...
  rest: {
    ...
    guidance: 'Focus on recovery habits, hydration, protein distribution, and micronutrient quality.',
...
export function mapNutritionStrategy(payload: unknown): NutritionStrategyStanding | null {
  ...
  const hydrationStatus: HydrationRingStatus =
    statusRaw === 'red' || statusRaw === 'yellow' || statusRaw === 'green'
      ? statusRaw
      : 'green';
...
export function fuelStateExplanation(input: {
  ...
  const title =
    input.state === 3 ? 'Performance' : input.state === 2 ? 'Steady' : 'Eco';
  return { title, ...copy };
}

/** Downsample dense wave points for a phone sparkline (keeps first/last). */
export function mapEnergyHorizonPoints(
  payload: unknown,
  maxPoints = 72
): EnergyWavePoint[] {
  const root = asRecord(payload);
...
export function mapActiveFuelFeed(payload: unknown): ActiveFuelFeedView {
  ...
  for (const item of recent.slice(0, 3)) {
    ...
    if (!r) continue;
    const name =
      typeof r.name === 'string' && r.name.trim() ? r.name.trim() : 'Logged food';
