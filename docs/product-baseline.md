...
| **Coach** | Chat |
| **More** | Your training (Activity history, Upcoming workouts, Events, Goals) · Account (Settings, Notifications, Subscription, Invite a friend) · Help · About · Sign out |
...
**More hosts:** four grouped sections — *Your training* (activity history, upcoming workouts (**separate from Plan** for now), **Events**, **Goals**), *Account* (Settings hub, notifications inbox, **Subscription**, **Invite a friend**), *Help*, *About* — plus a standalone Sign out. The profile card opens Athlete (body metrics / AI report / **activity glance** + primary-goal teaser → Goals hub, also via Today → name). **Invite a friend** is hosted/local only (QR + share link with Athlete A→B referral attribution, subtitle shows attribution count) and lives on **More only**; coaching `/join/{CODE}` relationships stay web. Rows that hand off to the browser are marked with an external-link glyph.

**Settings hub** is configuration only (no browsable lists), in four sections. *Data sources:* Apple Health / Health Connect (named per platform) · **Connected apps lite** (status + Connect/Fix/Manage via web handoff; disconnect/sync/ingest editors stay web). *Preferences:* Appearance (**this device**) · Units & locale · Notifications · Default log view (**this device**) · **Nutrition tracking** (tracking, metabolic, meal pattern, dietary constraints, fuel calibration, adaptive engine, hydration — web Profile → Nutrition parity). *Coaching:* Coach persona lite · Sports & thresholds lite. *Account:* **Subscription** (hosted current plan + Apple/Google purchase/restore/manage; Stripe manage via web) · Server · Manage account on the web · Export · Delete. Preferences follow the account unless tagged “This device”. Billing administration and zone editors stay web.
